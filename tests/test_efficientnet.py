...
def load_localization_model():
    ...
    checkpoint_path = os.path.join(
        project_root, 'weights', 'best_model_efficientnet_b4_bce_dice_large.pth'
    )
...
def create_three_panel_visualization(image, ground_truth_mask, prediction_mask,
    ...
    axes[2].set_title(
        f'EfficientNet-B4 U-Net (Large Dataset)\nIoU: {iou:.4f} | F1: {f1:.4f}',
...
def run_localization_test():
    print('EFFICIENTNET-B4 U-NET LARGE DATASET LOCALIZATION TEST')
    print('=' * 60)
    ...
    output_dir = os.path.join(project_root, 'test_results', 'efficientnet_b4_large_localization')
    ...
    for i, image_id in enumerate(test_images, 1):
        ...
        output_path = os.path.join(output_dir, f'efficientnet_b4_large_test_{i}.png')
