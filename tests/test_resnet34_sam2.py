...
def load_sam2_model(device):
    sam2_checkpoint = os.path.join(project_root, 'weights', 'sam2_hiera_large.pt')
    if not os.path.exists(sam2_checkpoint):
        print(f'ERROR: SAM2 model not found: {sam2_checkpoint}')
        return None

    try:
        import sam2 as sam2_pkg
        sam2_dir = os.path.dirname(sam2_pkg.__file__)
        config_path = os.path.join(sam2_dir, 'configs', 'sam2', 'sam2_hiera_l.yaml')

        if not os.path.exists(config_path):
            config_path = os.path.join(sam2_dir, 'configs', 'sam2_hiera_l.yaml')

        if not os.path.exists(config_path):
            print(f'ERROR: SAM2 config not found. Searched: {config_path}')
        ...
        print(f'SUCCESS: SAM2 loaded from {sam2_checkpoint}')
        return predictor
...
def refine_with_sam2(predictor, image, bboxes):
    if len(bboxes) == 0:
...
def create_visualization(image, gt_mask, unet_mask, sam2_mask,
                          output_path, iou_unet, f1_unet, iou_sam2, f1_sam2):
    ...
    axes[3].imshow(sam2_mask, cmap='gray', vmin=0, vmax=255)
    axes[3].set_title(
        f'U-Net + SAM2 Refined\nIoU: {iou_sam2:.4f} | F1: {f1_sam2:.4f}',
        fontsize=12, fontweight='bold'
    )
    axes[3].axis('off')
...
def run_pipeline():
    print('RESNET34 U-NET + SAM2 REFINEMENT PIPELINE')
    print('=' * 60)
    ...
    output_dir = os.path.join(project_root, 'test_results', 'resnet34_sam2')
    ...
    for i, image_id in enumerate(test_images, 1):
        ...
        sam2_mask = refine_with_sam2(sam2_predictor, image, bboxes)
        iou_sam2, f1_sam2 = compute_metrics(gt_mask, sam2_mask)

        all_iou_unet.append(iou_unet)
        all_f1_unet.append(f1_unet)
        all_iou_sam2.append(iou_sam2)
        all_f1_sam2.append(f1_sam2)

        print(f'  U-Net  → IoU: {iou_unet:.4f} | F1: {f1_unet:.4f}')
        print(f'  SAM2   → IoU: {iou_sam2:.4f} | F1: {f1_sam2:.4f}')

        output_path = os.path.join(output_dir, f'resnet34_sam2_{i}.png')
        create_visualization(
            image, gt_mask, unet_mask, sam2_mask,
            output_path, iou_unet, f1_unet, iou_sam2, f1_sam2
        )
    ...
    if all_iou_unet:
        ...
        print(f'\nResNet34 U-Net + SAM2:')
        print(f'  Mean IoU : {np.mean(all_iou_sam2):.4f} ± {np.std(all_iou_sam2):.4f}')
        print(f'  Mean F1  : {np.mean(all_f1_sam2):.4f} ± {np.std(all_f1_sam2):.4f}')
        delta_iou = np.mean(all_iou_sam2) - np.mean(all_iou_unet)
        delta_f1 = np.mean(all_f1_sam2) - np.mean(all_f1_unet)
        print(f'\nMejora SAM2:')
