...
def load_localization_model():
    ...
    checkpoint_path = os.path.join(project_root, 'weights', 'best_model_bce_dice.pth')
...
def create_three_panel_visualization(image, ground_truth_mask, prediction_mask,
    ...
    axes[0].imshow(image)
    axes[0].set_title('Original PRE-Disaster Satellite Image', fontsize=14, fontweight='bold')
    axes[0].axis('off')
    ...
    axes[2].set_title(
        f'SmallUNet BCE+Dice Prediction\nIoU: {iou:.4f} | F1: {f1:.4f}',
...
def run_localization_test():
    print('SMALLUNET BCE+DICE LOCALIZATION TEST')
    print('=' * 60)
    ...
    output_dir = os.path.join(project_root, 'test_results', 'localization_bce_dice2')
    ...
    for i, image_id in enumerate(test_images, 1):
        ...
        output_path = os.path.join(output_dir, f'bce_dice_test_{i}.png')
